...
export default function MapScreen() {
  ...
  useEffect(() => {
    async function askPermissions() {
      ...
      if (status === 'granted') {
        ...
        socket.emit("sendPositionLatitude", currentLatitude)
        socket.emit("sendPositionLongitude", currentLongitude)
  ...
  useEffect(() => { 
    ...
    socket.on('sendPositionLatitude', (newLat)=> {
      setuserLatitude(newLat);
    });
    socket.on('sendPositionLongitude', (newLong)=> {
      setuserLongitude(newLong);
    });
  ...
  var handleSubmit = () => {
    var copy = [...listPOI, {longitude: tempPOI.longitude, latitude: tempPOI.latitude, titre: titrePOI, description: descPOI } ];

    setListPOI([...listPOI, {longitude: tempPOI.longitude, latitude: tempPOI.latitude, titre: titrePOI, description: descPOI } ]);
    AsyncStorage.setItem("POI", JSON.stringify(copy) )
